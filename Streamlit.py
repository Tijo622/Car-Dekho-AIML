...
img = Image.open("green-jeep-pickup-truck-drives-through-desert-landscape.jpg")
st.set_page_config(page_title = 'Cardekho Resale Price Prediction', page_icon = img,layout = 'wide')
st.image("green-jeep-pickup-truck-drives-through-desert-landscape.jpg", caption="CarDekho")
...
@st.cache_data
def data():
        df = pd.read_csv('Filtered_Car_Dekho.csv')
...
if button == True:
 result = model.predict([[inv_trans(km_driven), Transmission_Type[transmission], Car_Model[car_model], model_year, engine_cc, mileage, Location[location]]]) # type: ignore
